#!/usr/bin/env bash
# Browser interop leg: real Chrome vs the in-tree WT echo server.
#
...
#             2 = setup failure (missing browser, no READY, no RESULT).
set -euo pipefail
...
if ! BROWSER="$(find_browser)"; then
    echo "no Chrome found (set CHROME_BIN, or install google-chrome)" >&2
    exit 2
fi
...
# In-run P-256 cert, 10-day validity: short enough for Chrome's 14-day
# serverCertificateHashes rule, so the SAME cert also serves the manual
# hash-pinning recipe documented in the README - one recipe, two trust paths.
...
# Chrome trusts by SPKI hash, not cert hash: the flag pins the public key
# so it keeps working even if the cert were re-issued around the same key.
SPKI="$(openssl x509 -in "$WORK/cert.pem" -pubkey -noout \
    | openssl pkey -pubin -outform der \
    | openssl dgst -sha256 -binary | base64)"

# All eras advertised: shipped Chrome must land on draft02 on its own -
...
# WPT's Chrome recipe: developer mode + SPKI allowlist. Deliberately NOT
# serverCertificateHashes (opaque failures in CI) and NOT
# --origin-to-force-quic-on (unnecessary for WebTransport).
"$BROWSER" \
    --headless=new \
    --no-sandbox \
    --disable-gpu \
...
echo "PASS chrome browser interop (era=draft02)"
